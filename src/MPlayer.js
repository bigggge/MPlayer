...
!function () {
  ...
  MPlayer.prototype.init = function () {
    ...
    this.spectrumEl = el.getElementsByClassName('player-spectrum')[0];
  ...
  function getParsedLrcArr (text) {
    ...
    for (let i = 0, lyricLen = lyric.length; i < lyricLen; i++) {
      ...
      // match lrc text
      const lrcText = lyric[i].replace(/\[(\d{2}):(\d{2})\.(\d{2,3})]/g, '').
        replace(/^\s+|\s+$/g, '');
      if (lrcTimes !== null) {
  ...
  // TODO Uncaught DOMException: Failed to execute 'createMediaElementSource' on 'BaseAudioContext': HTMLMediaElement already connected previously to a different MediaElementSourceNode.
  MPlayer.prototype.initSpectrum = function () {
    ...
    const audioCtx = new (window.AudioContext || window.webkitAudioContext)(); // declare new audio context
